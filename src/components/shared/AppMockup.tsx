...
function IPhoneMockup({ accentColor, size }: { accentColor: string; size: "sm" | "md" | "lg" }) {
  const dims = size === "lg"
  ...
  return (
    <div className={cn("relative shrink-0", dims.w, dims.h, dims.radius, dims.border, "border-foreground/20 bg-card overflow-hidden shadow-2xl")}>
      {/* Dynamic island */}
      ...
      <div
        ...
        style={{ background: `linear-gradient(160deg, ${accentColor}35 0%, ${accentColor}10 45%, transparent 75%)` }}
      />
      ...
      {/* Home indicator */}
      <div className="absolute bottom-2 left-1/2 -translate-x-1/2 w-1/3 h-1 rounded-full bg-foreground/20" />
    </div>
  );
}

function BrowserMockup({ accentColor, size }: { accentColor: string; size: "sm" | "md" | "lg" }) {
      ...
      <div className="flex h-full">
        ...
        <div className="flex-1 p-3">
          ...
          <div className="grid grid-cols-3 gap-2 mb-3">
            {[0, 1, 2].map((i) => (
              <div
                ...
                style={{ backgroundColor: i === 0 ? `${accentColor}25` : `${accentColor}10`, border: `1px solid ${accentColor}${i === 0 ? "40" : "20"}` }}
              >
          ...
          <div
            ...
            style={{ background: `linear-gradient(135deg, ${accentColor}15 0%, transparent 70%)`, border: `1px solid ${accentColor}25` }}
          >
            <div className="flex items-end gap-1 h-12">
              {[45, 65, 40, 80, 55, 90, 70, 60].map((h, i) => (
                <div
                  key={i}
                  className="flex-1 rounded-t-sm"
                  style={{ height: `${h}%`, backgroundColor: `${accentColor}${i === 5 ? "cc" : "50"}` }}
                />
              ))}
            </div>
          </div>
...
export function AppMockup({ type, accentColor, className, size = "md" }: AppMockupProps) {
  if (type === "ios") {
    return (
      <div className={cn("flex items-center justify-center", className)}>
        <IPhoneMockup accentColor={accentColor} size={size} />
      </div>
    );
  }
  ...
  return (
    <div className={cn("w-full", className)}>
      <BrowserMockup accentColor={accentColor} size={size} />
    </div>
  );
}
